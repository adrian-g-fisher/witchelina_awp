...
# Output directory
plot_dir <- "C:\\Users\\Adrian\\OneDrive - UNSW\\Documents\\publications\\preparation\\witchelina_awp_grazing\\model_results_random_intercept" # Directory to save the plots
...
for (colname in columns) {
  ...
  model <- try(glmmTMB(formula=as.formula(paste(colname, " ~ Distance + (1 | Paddock)")), data=df), silent=TRUE)
	df$predicted_values <- predict(model)
	
	# Get fixed effects
	fixed_effects <- extract_fixed_effects(model)
	write_csv(fixed_effects, paste0(plot_dir, "/fixed_effects_", season, ".csv"))
	
  # Get random effects for each paddock
	random_effects <- extract_random_coefs(model)
  colnames(random_effects)[colnames(random_effects) == "group"] <- "Paddock" 
  random_effects <- merge(random_effects, summary_df, by="Paddock")
  
  missing <- setdiff(paddock_list, summary_df$Paddock)
  
  for (p in missing) {
    new_1 <- add_row(df, Paddock = p)
    new_2 <- add_row(random_effects, Paddock = p, n = 0)
    df <- new_1
    random_effects <- new_2
  }
  
  write_csv(random_effects, paste0(plot_dir, "/random_effects_", season, ".csv"))
